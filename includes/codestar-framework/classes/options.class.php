...
if ( ! class_exists( 'CSF_Options' ) ) {
  class CSF_Options extends CSF_Abstract {
    ...
    public $args         = array(
      ...
      // framework title
      'framework_title'         => '主题选项 <small>by ThemeMee</small>',
      'framework_class'         => '',
    ...
    public function set_options( $ajax = false ) {
      ...
      $nonce     = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( $_POST[ $noncekey ] ) : '';
      ...
      if ( wp_verify_nonce( $nonce, 'csf_options_nonce' ) ) {
        ...
        if ( ! empty( $transient['csf_import_data'] ) ) {
          ...
          $importing   = true;
          $import_data = json_decode( wp_unslash( trim( $transient['csf_import_data'] ) ), true );
      ...
      echo '<div class="csf csf-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<div class="csf-header-right">';
            ...
            echo ( $this->args['show_form_warning'] ) ? '<div class="csf-form-result csf-form-warning">'. esc_html__( '设置已更改，请注意保存！', 'csf' ) .'</div>' : '';

            echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="csf-expand-all" title="'. esc_html__( '显示所有选项', 'csf' ) .'"><i class="fas fa-outdent"></i></div>' : '';
            ...
            echo '<input type="submit" name="'. esc_attr( $this->unique ) .'[_nonce][save]" class="button button-primary csf-top-save csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( '保存', 'csf' ) .'" data-save="'. esc_html__( '保存中...', 'csf' ) .'">';
            echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( '重置部分', 'csf' ) .'" data-confirm="'. esc_html__( '您确定要重置此部分的选项吗？', 'csf' ) .'">' : '';
            echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. esc_html__( '重置全部', 'csf' ) .'" data-confirm="'. esc_html__( '您确定要重置所有选项吗？', 'csf' ) .'">' : '';
            echo '</div>';
          ...
          echo '<div class="csf-content">';
            ...
            foreach ( $this->pre_sections as $section ) {
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                echo '<div class="csf-no-option csf-text-muted">'. esc_html__( '开发人员未提供任何选项。', 'csf' ) .'</div>';
